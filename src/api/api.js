...
//     const response = await makeRequest(`${BASE_URL}/datastores`, options);
//     const data = await response.json();
//     return { success: true, data };
//   } catch (error) {
//     console.error('Credentials verification failed:', error);
...
const getAuthHeaders = (username, password) => {
  ...
  const { username: storedUsername, password: storedPassword } = JSON.parse(authData);
  const base64Credentials = Buffer.from(`${storedUsername}:${storedPassword}`).toString("base64");
  return {
    "Authorization": `Basic ${base64Credentials}`,
    "Content-Type": "application/json",
    "Accept": "application/json",
...
// Make API request with self-signed SSL handling
const makeRequest = async (url, options) => {
  const fetch = await getFetch(); // Dynamically import fetch
  try {
    const response = await fetch(url, {
      ...options,
      agent, // Allow self-signed SSL
    });
