...
function DatastoreDetail() {
  ...
  // Use useLocalStorage for persistent preferences
  ...
  const debouncedSearch = useDebounce(searchInput, config.SEARCH_DEBOUNCE_MS);
    ...
    const loadFiles = async () => {
      ...
      try {
        ...
        if (id.toLowerCase().includes('mock')) {
          ...
          const fromRows = ((currentPage - 1) * config.ITEMS_PER_PAGE).toString();
          const rows = config.ITEMS_PER_PAGE.toString();
  ...
  }, [id, currentPage, sortConfig, setVisibleColumns]);

  // Optimize filtered files calculation
  const filteredFiles = useMemo(() => {
    ...
    if (searchTerm || filters.status.length || filters.fileType.length || 
      ...
      const fileTypeSet = new Set(filters.fileType);
      const directionSet = new Set(filters.direction);
      
      result = result.filter(file => {
        // Quick rejection checks first
        if (statusSet.size && !statusSet.has(file.status)) return false;
        if (fileTypeSet.size && !fileTypeSet.has(file.fileType)) return false;
        if (directionSet.size && !directionSet.has(file.direction)) return false;
        ...
        if (filters.clientName && (!file.clientName || 
            !file.clientName.toLowerCase().includes(filters.clientName.toLowerCase()))) {
          return false;
        }
    ...
    return result;
  }, [files, debouncedSearch, filters, sortConfig]);
  ...
  const downloadExcel = useCallback(async () => {
    try {
      ...
      for (let i = 0; i < filteredFiles.length; i += config.EXPORT_BATCH_SIZE) {
        const batch = filteredFiles.slice(i, i + config.EXPORT_BATCH_SIZE);
  ...
  }, [filteredFiles, visibleColumns, id, addError]);
  ...
  const visibleRange = {
    start: (currentPage - 1) * config.ITEMS_PER_PAGE,
    end: currentPage * config.ITEMS_PER_PAGE
  };
